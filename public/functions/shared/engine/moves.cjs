const { otherSide, fileOf, rankOf, inBounds, sq } = require("./util.cjs");

const FILES = ["a","b","c","d","e","f","g","h"];

function isSquareOccupied(state, square){
...
function generateMovesStandard(state, pieceId){
  ...
  if(p.type === "N"){
    const deltas = [[1,2],[2,1],[2,-1],[1,-2],[-1,-2],[-2,-1],[-2,1],[-2,1],[-1,2]];
    // fix duplicate [-2,1] by building explicitly
    const deltas2 = [[1,2],[2,1],[2,-1],[1,-2],[-1,-2],[-2,-1],[-2,1],[-1,2]];
    for(const [df, dr] of deltas2){
...
function attacksSquare(state, attackerPieceId, targetSquare){
...
function findKingSquare(state, side){
  const kid = `${side}_K`;
